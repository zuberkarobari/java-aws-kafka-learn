...
with open('c:/zuber-projects/DONE/study-site/js/topics.config.js', 'r', encoding='utf-8') as f:
    content = f.read()

new_order = """export const TOPIC_ORDER = [
  // ── Fundamentals ──────────────────────────────────────────────
  'java/java-basics',
...
start = content.find('export const TOPIC_ORDER = [')
end = content.find('];', start) + 2

new_content = content[:start] + new_order + content[end:]

# Also update TOPIC_META keys
new_content = new_content.replace("'java-basics': {", "'java/java-basics': {")
new_content = new_content.replace("'multithreading': {", "'java/multithreading': {")
new_content = new_content.replace("'springboot': {", "'spring-boot/springboot': {")

with open('c:/zuber-projects/DONE/study-site/js/topics.config.js', 'w', encoding='utf-8') as f:
    f.write(new_content)
